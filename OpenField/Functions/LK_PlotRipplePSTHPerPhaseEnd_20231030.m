function f = LK_PlotRipplePSTHPerPhaseEnd_20231030(dt)
%
% LK_PlotRipplePSTHPerPhaseEnd_20231030 plots the ripple PSTH and
% time-resolved average ripple rate per trial phase. Good and bad trials
% are separated. Data is locked to the end of the trial phases.
%
% Lukas Kunz, 2023
...
for iM = 1:numel(m)
    pa = patch([dt.beh.PSTHEndTime, fliplr(dt.beh.PSTHEndTime)], ...
        [m{iM} + ste{iM}, fliplr(m{iM} - ste{iM})], dt.beh.trialPhasesColor(dt.iPhase, :), ...
        'EdgeColor', 'none', 'FaceAlpha', 0.5);
    p = plot(dt.beh.PSTHEndTime, m{iM}, 'Color', dt.beh.trialPhasesColor(dt.iPhase, :));
    if iM == 1
        set(pa, 'FaceColor', [0.5, 0.5, 0.5]); % bad trials in gray
        set(p, 'Color', [0.5, 0.5, 0.5]);
    end
end
% plot significance
sigTime = zeros(1, numel(dt.beh.PSTHEndTime));
sigTime(ismember(dt.beh.PSTHEndTime, dt.outFT.time))  = dt.outFT.mask;
LK_SigLine(dt.beh.PSTHEndTime, [max(dt.thisParam.yLim); max(dt.thisParam.yLim) - 0.05 * range(dt.thisParam.yLim)], sigTime == 1);
% enhance axes
set(gca, ...
...
for iTrial = 1:size(dt.allPSTH, 1)
    myp = plot(dt.beh.PSTHEndTime(dt.allPSTH(iTrial, :) > 0), repmat(iTrial, 1, sum(dt.allPSTH(iTrial, :) > 0)), '.', ...
